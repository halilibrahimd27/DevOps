...
**EN kapsama:** 108 site sayfası / 334 TR temel sayfası ≈ **%32.3** (Aşama B eşiği %60).
...
+ P4 slice-2: 5 deep-dive + P4 slice-3: 5 deep-dive + P4 slice-4: 5 deep-dive + P4 slice-5: 5 deep-dive = 108. Kök `README.en.md` GitHub-only, siteye stage edilmez → oran dışı.)
...
| **P4** | Kalan içerik | 🟡 **slice-5 hazır** (2026-07-24) — 5 `.en.md`, `06-GitOps/*` kalan 4 (#21–24) + `05-Kubernetes/*` ilk 1 (#25): `06-GitOps/ArgoCD-Setup`, `06-GitOps/Flux-vs-ArgoCD`, `06-GitOps/Helm-vs-Kustomize-vs-Raw`, `06-GitOps/Secrets-in-GitOps`, `05-Kubernetes/Debugging-Pods`. 5 paralel çeviri subagent, dosya başına bir (sonnet), oturmuş genişletilmiş ruleset + aynı-klasör gold-standard referans (`06-GitOps/App-of-Apps-Pattern.en.md`; Debugging-Pods için ek `08-Security/Kubernetes-Hardening.en.md`) → remediation gerekmedi. Plain/untagged blok prose çevrildi (Secrets decision-tree `EVET`→`YES`/`HAYIR`→`NO`, Helm dizin-layout etiketleri, Debugging bash yorumu `# Log from the previous crash (critical for CrashLoopBackOff)`, checklist `[ ]`; `%X`→`X%`, `dk`→`min`, `$X/ay`→`$X/mo`); K8s status/reason string'leri (`CrashLoopBackOff`/`OOMKilled`/`ImagePullBackOff`/`Pending`) + Flux/ArgoCD CRD adları (Kustomization/HelmRelease/GitRepository/Application/ApplicationSet) + Secrets tool/CRD (SealedSecret/ExternalSecret/SecretStore/SOPS/ESO) + Helm/Kustomize field (`values`/`kustomization.yaml`) + `{{...}}` Go-template + komut + link-target verbatim. Bağımsız orchestrator paritesi: başlık 5/5 (29/24/36/46/84), tablo 5/5 (26/44/37/18/19), fence 5/5 (40/10/38/40/34), satır deltası 0/+1/0/+2/+1 (prose-wrap), gerçek Türkçe kalıntısı **0** (`[ışğİŞĞ]` excl `/var/` + diakritiksiz TR-fonksiyon-kelime taraması da temiz, 5/5), link locale-eksiz (0 sızıntı), positioning/pazarlama (TR+EN, `ROI`/`guarantee` dahil) **0 hit**. qa exit 0, iki-locale build hatasız, 5 sayfa `/en/…/` İngilizce render (EN-marker 384/338/322/447/341), `_planning` sızmadı. **`06-GitOps` artık tam twin (7/7)**, `05-Kubernetes` başladı (README + Debugging-Pods = 2/7). **Slice-1..4 önceki turlar.** **Kalan P4 çok turlu** — sıra slice-6 (`05-Kubernetes/*` kalan 5 → `04-Containers/*` → `03-IaC/*` → kalan 00-21). |
...
| 26+ | `05-Kubernetes/*` kalan (HPA-VPA-KEDA, Multi-Tenancy-Patterns, Production-Checklist, Resource-Limits-Guide, Upgrade-Strategy), `04-Containers/*`, `03-IaC/Terraform-Best-Practices.md` → sonra kalan 00-21 → `16-Cheatsheets/` → `17-Templates/` → `21-Field-Notes/` | 6+ |
