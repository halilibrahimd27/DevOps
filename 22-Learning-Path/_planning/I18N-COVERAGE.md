...
**EN kapsama:** 128 site sayfası / 334 TR temel sayfası ≈ **%38.3** (Aşama B eşiği %60).
...
+ P4 slice-2: 5 deep-dive + P4 slice-3: 5 deep-dive + P4 slice-4: 5 deep-dive + P4 slice-5: 5 deep-dive + P4 slice-6: 5 deep-dive + P4 slice-7: 5 deep-dive + P4 slice-8: 5 deep-dive + P4 slice-9: 5 deep-dive = 128. Kök `README.en.md` GitHub-only, siteye stage edilmez → oran dışı.)
...
| **P4** | Kalan içerik | 🟡 **slice-9 hazır** (2026-07-24) — 5 `.en.md`: `00-Culture/Team-Topologies` (#41 → `00-Culture` **tam twin 8/8**), `02-CI-CD/Caching-Strategies` (#42), `02-CI-CD/GitHub-Actions-Recipes` (#43), `02-CI-CD/GitLab-CI-Recipes` (#44), `02-CI-CD/Mobile-CICD-Flutter` (#45) — #41 dilim planından, #42–45 = kalan 00-21 deep-dive'ları **klasör sırasıyla** (01-Linux-Networking tam twin → atlandı; `02-CI-CD` sırada, Pipeline-Patterns + README önceki turlarda twin'liydi). 5 paralel çeviri subagent, dosya başına bir (sonnet), oturmuş genişletilmiş ruleset + aynı-klasör gold-standard referans (`00-Culture/README.en.md`+`On-Call-Playbook.en.md`, `02-CI-CD/Pipeline-Patterns.en.md`+`README.en.md`) → remediation gerekmedi, ilk çeviride doğru. Plain/untagged blok prose çevrildi (org-diyagram etiketleri; kod-yorumu `# required for OIDC`/`# automatic node_modules cache`/`# cancel the old run when a new commit arrives`/`# Or:`/`# -count=1 disables cache`; keystore warning `[ ]`/`⚠️` prose; `%X`→`X%`, `dk`→`min`, `hafta`→`week`); **Mobile-CICD-Flutter iç ToC anchor'ları yeniden üretildi** (8/8 GitHub-slug, `#1-accounts-and-memberships` … `#8-total-cost-breakdown` → çevrilmiş `## N.` başlıklarına çözülüyor). Verbatim korundu: GH-Actions YAML (`uses:`/`with:`/`permissions:`/`${{ }}`/`secrets.`, SHA-pin/`<VERSION>` policy), GitLab CI (`$CI_*`/`stages:`/`rules:`/`cache:`/`&`/`*` anchor), cache token (`actions/cache@`/`--mount=type=cache`/`--cache-from`/`gha`), Flutter/Fastlane/Gradle (`keytool`/`build.gradle`/`key.properties`/`.jks`/`.p12`/`.mobileprovision`/`pubspec.yaml`), bundle-id `com.onmuhasebe.mobile` (kaynak-sadık örnek, 5/5 kaynakta da var), Conway's Law/Skelton & Pais/Team Topologies proper-noun + takım-türü adları (stream-aligned/enabling/complicated-subsystem/platform), link-target locale-eksiz. Bağımsız orchestrator paritesi: başlık 5/5 (30/44/22/24/93), tablo 5/5 (19/13/12/13/25), fence 5/5 (4/56/32/28/52, twin==source), satır deltası 0/0/0/0/-1 (Mobile trailing-nl), gerçek Türkçe kalıntısı **0** (`[ışğİŞĞçöü]` excl path/Istanbul + diakritiksiz TR-fonksiyon-kelime temiz, 5/5; render `<article>` body-TR=0), link locale-eksiz (0 sızıntı), positioning/pazarlama (TR+EN) **0 hit**. qa exit 0, iki-locale build hatasız, 5 sayfa İngilizce render (`site/en/…`, body-TR=0), TR root default korundu, `_planning` sızmadı. EN kapsama %36.8 → %38.3 (128/334). **`00-Culture` tam twin (8/8); `02-CI-CD` 6/8** (bu tur 4: Caching/GH-Actions/GitLab/Mobile + Pipeline-Patterns[P3] + README[P2]; kalan `Pipeline-Performance` + `Reusable-Workflows` → slice-10). **Slice-1..8 önceki turlar.** **Kalan P4 çok turlu** — sıra slice-10 (`02-CI-CD/Pipeline-Performance` + `Reusable-Workflows` + `03-IaC/*` kalan 5 klasör sırasıyla). |
...
| 46+ | sonra kalan 00-21 deep-dive'ları klasör sırasıyla (`02-CI-CD` kalan 2: `Pipeline-Performance`+`Reusable-Workflows`, sonra `03-IaC/*` kalan 5, `07-Observability/*`, `08-Security/*`, …) → `16-Cheatsheets/` (kalan 8) → `17-Templates/` index'leri → en son `21-Field-Notes/` | 10+ |
