...
**EN kapsama:** 113 site sayfası / 334 TR temel sayfası ≈ **%33.8** (Aşama B eşiği %60).
...
+ P4 slice-2: 5 deep-dive + P4 slice-3: 5 deep-dive + P4 slice-4: 5 deep-dive + P4 slice-5: 5 deep-dive + P4 slice-6: 5 deep-dive = 113. Kök `README.en.md` GitHub-only, siteye stage edilmez → oran dışı.)
...
| **P4** | Kalan içerik | 🟡 **slice-7 hazır** (2026-07-24) — 5 `.en.md`, `04-Containers/*` kalan 6'nın ilk 5 (#31–35): `04-Containers/BuildKit-Tips`, `04-Containers/Container-vs-WASM`, `04-Containers/Distroless-and-Chainguard`, `04-Containers/Dockerfile-Best-Practices`, `04-Containers/Image-Signing-Cosign` (6. `Multi-Stage-Builds` → slice-8; README önceki turda twin'liydi → `04-Containers` bu turla 6/7). 5 paralel çeviri subagent, dosya başına bir (sonnet), oturmuş genişletilmiş ruleset + aynı-klasör gold-standard referans (`04-Containers/README.en.md`) → remediation gerekmedi, ilk çeviride doğru. Plain/untagged blok prose çevrildi (Dockerfile code-yorumu `# ❌ Kötü`→`# ❌ Bad`/`# ✅ İyi`→`# ✅ Good`, anti-pattern tablo hücreleri `Çok`→`Many`/`kaçın`→`avoid`, BuildKit `# Single build`/`# Daemon-wide` yorumları, `[ ]` checklist label metni; `%X`→`X%`, `dk`→`min`, `hafta`→`week`); image ref (`gcr.io/distroless/static-debian12`, `cgr.dev/chainguard/*`, `ubuntu:22.04`, `:20-alpine`) + Dockerfile talimatları (`FROM`/`RUN`/`COPY`/`USER`/`HEALTHCHECK`) + BuildKit token (`--mount=type=cache`/`secret`/`ssh`, `buildx`, `--platform linux/amd64,linux/arm64`, `DOCKER_BUILDKIT`) + Wasm ekosistemi (`Spin`/`wasmCloud`/`WASI`/`wasmtime`/`.wasm`) + Cosign/Sigstore (`cosign sign/verify`, `Fulcio`/`Rekor`, `keyless`, `SBOM`, `Kyverno verifyImages`, `validationFailureAction: Enforce`) + digest/SHA-pin + link-target verbatim. Bağımsız orchestrator paritesi: başlık 5/5 (45/29/37/68/23), tablo 5/5 (21/55/33/29/16), fence 5/5 (58/22/20/50/20, twin==source), satır deltası 0/+1/0/0/+1 (trailing-nl/prose-wrap), gerçek Türkçe kalıntısı **0** (article body-TR=0, 5/5; `[ışğİŞĞçöü]` excl path/Istanbul + diakritiksiz TR-fonksiyon-kelime temiz), link locale-eksiz (0 sızıntı), positioning/pazarlama (TR+EN) **0 hit** (bu 5 kaynak artifact yoğun; ROI/guarantee prose'u geçmiyor). qa exit 0, iki-locale build hatasız, 5 sayfa `/en/04-Containers/…/` İngilizce render (EN-marker 404/315/394/473/355, body-TR=0), `_planning` sızmadı. EN kapsama %33.8 → %35.3 (118/334). **Slice-1..6 önceki turlar.** **Kalan P4 çok turlu** — sıra slice-8 (`04-Containers/Multi-Stage-Builds` + `03-IaC/Terraform-Best-Practices.md` + kalan 00-21). |
...
| 38+ | sonra kalan 00-21 deep-dive'ları klasör sırasıyla → `16-Cheatsheets/` (kalan 8) → `17-Templates/` index'leri → en son `21-Field-Notes/` | 8+ |
