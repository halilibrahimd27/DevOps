...
**EN kapsama:** 103 site sayfası / 334 TR temel sayfası ≈ **%30.8** (Aşama B eşiği %60).
...
+ P4 slice-2: 5 deep-dive + P4 slice-3: 5 deep-dive + P4 slice-4: 5 deep-dive = 103. Kök `README.en.md` GitHub-only, siteye stage edilmez → oran dışı.)
...
| **P4** | Kalan içerik | 🟡 **slice-4 hazır** (2026-07-23) — 5 `.en.md`, count-4 "yakın-kaçıran" kuyruğu (#16–18) + `06-GitOps/*` başı (#19–20): `12-FinOps/Storage-Cost-Optimization`, `10-Databases-Production/Zero-Downtime-Migrations`, `07-Observability/SLO-Engineering`, `06-GitOps/App-of-Apps-Pattern`, `06-GitOps/ApplicationSet-Patterns`. 5 paralel çeviri subagent, dosya başına bir (sonnet), oturmuş genişletilmiş ruleset + aynı-/ilgili-klasör gold-standard referans (`Cloud-Cost-Allocation.en.md`, `Backup-Restore-Patterns.en.md`, `Prometheus-Best-Practices.en.md`) → remediation gerekmedi. Plain/untagged blok prose çevrildi (ASCII app-hierarchy/generator fan-out tree etiketleri, PromQL yorumu `# 5-minute error rate`/`# 1 = normal rate, 14 = 14x burn`, checklist `[ ]`; `%99.9`→`99.9%` yüzde konvansiyonu, SLO birimleri `43,200 dk`→`43,200 min`); ArgoCD `kind: Application`/`ApplicationSet` + generator adları (`list`/`cluster`/`git`/`matrix`) + `{{...}}` Go-template placeholder + storage type (gp3/io2/st1) + PromQL + SQL identifier + komut + link-target verbatim. Bağımsız orchestrator paritesi: başlık 5/5 (37/28/33/28/27), tablo 5/5 (25/23/17/19/12), fence 5/5 (30/40/26/24/38), satır deltası 0/0/0/0/+1 (ApplicationSet prose-wrap), gerçek Türkçe kalıntısı **0** (`[ışğİŞĞ]` excl `/var/` + diakritiksiz TR-fonksiyon-kelime taraması da temiz), link locale-eksiz (0 sızıntı), positioning temiz, **pazarlama/ROI grep 0 hit**. qa exit 0, iki-locale build hatasız, 5 sayfa `/en/…/` İngilizce render (EN-marker 227/263/235/248/246). **`12-FinOps` tam twin (8/8)**, `06-GitOps` başladı (2/7). **Slice-1/2/3 önceki turlar.** **Kalan P4 çok turlu** — sıra slice-5 (`06-GitOps/*` kalan 5 → `05-Kubernetes/*` → `04-Containers/*` → `03-IaC/*` → kalan 00-21). |
...
| 21+ | `06-GitOps/*` kalan (ArgoCD-Setup, Flux-vs-ArgoCD, Helm-vs-Kustomize-vs-Raw, Secrets-in-GitOps), `05-Kubernetes/*`, `04-Containers/*`, `03-IaC/Terraform-Best-Practices.md` → sonra kalan 00-21 → `16-Cheatsheets/` → `17-Templates/` → `21-Field-Notes/` | 5+ |
