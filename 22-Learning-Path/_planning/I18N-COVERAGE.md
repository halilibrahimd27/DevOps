...
**EN kapsama:** 123 site sayfası / 334 TR temel sayfası ≈ **%36.8** (Aşama B eşiği %60).
...
+ P4 slice-2: 5 deep-dive + P4 slice-3: 5 deep-dive + P4 slice-4: 5 deep-dive + P4 slice-5: 5 deep-dive + P4 slice-6: 5 deep-dive + P4 slice-7: 5 deep-dive + P4 slice-8: 5 deep-dive = 123. Kök `README.en.md` GitHub-only, siteye stage edilmez → oran dışı.)
...
| **P4** | Kalan içerik | 🟡 **slice-8 hazır** (2026-07-24) — 5 `.en.md`: `04-Containers/Multi-Stage-Builds` (#36 → `04-Containers` **tam twin 7/7**), `03-IaC/Terraform-Best-Practices` (#37), `00-Culture/Documentation-Culture` (#38), `00-Culture/DORA-SPACE-Metrics` (#39), `00-Culture/On-Call-Playbook` (#40) — #36–37 dilim planından, #38–40 = kalan 00-21 deep-dive'ları **klasör sırasıyla** (00-Culture ilk). 5 paralel çeviri subagent, dosya başına bir (sonnet), oturmuş genişletilmiş ruleset + aynı-klasör gold-standard referans (`04-Containers/Dockerfile-Best-Practices.en.md`, `03-IaC/README.en.md`, `00-Culture/Blameless-Postmortem-Template.en.md`) → remediation gerekmedi, ilk çeviride doğru. Plain/untagged blok prose çevrildi (Dockerfile code-yorumu `# Build stage`/`# Runtime stage`, ADR/RFC ```markdown template prose `## Context`/`## Decision`/`## Consequences` + `Superseded`/`Accepted` durum etiketleri, DORA tablo prose hücreleri `Elite/High/Medium/Low` + `Daily-weekly`, on-call runbook `[ ]` checklist + escalation-flow etiketleri; `%99.99`→`99.99%`, `dk`→`min`, `hafta`→`week`); HCL/Terraform (`resource`/`module`/`backend "s3"`/`for_each`/`lifecycle`, `terraform plan/apply`) + Dockerfile talimatları (`FROM`/`COPY --from=builder`/`USER nonroot:nonroot`/`ENTRYPOINT`) + image ref (`gcr.io/distroless/static-debian12`, `golang:1.23`) + DORA/SPACE metric adları (`Deployment Frequency`/`Lead Time`/`Change Failure Rate`/`MTTR`) + tool adı (PagerDuty/Opsgenie/Backstage/ADR/RFC) + SEV-1..4 + link-target verbatim. Bağımsız orchestrator paritesi: başlık 5/5 (35/53/42/26/36), tablo 5/5 (20/20/19/31/12), fence 5/5 (38/34/14/12/12, twin==source), satır deltası 0/0/+1/0/0 (Documentation prose-wrap), gerçek Türkçe kalıntısı **0** (`[ışğİŞĞçöü]` excl path/Istanbul + diakritiksiz TR-fonksiyon-kelime temiz, 5/5), link locale-eksiz (0 sızıntı), positioning/pazarlama (TR+EN) **0 hit**. qa exit 0, iki-locale build hatasız, 5 sayfa İngilizce render (EN-marker-lines 149/104/123/128/136), TR root default korundu, `_planning` sızmadı. EN kapsama %35.3 → %36.8 (123/334). **`04-Containers` tam twin (7/7); `00-Culture` 4/8.** **Slice-1..7 önceki turlar.** **Kalan P4 çok turlu** — sıra slice-9 (`00-Culture/Team-Topologies` + `02-CI-CD/*` klasör sırasıyla). |
...
| 42+ | sonra kalan 00-21 deep-dive'ları klasör sırasıyla (`02-CI-CD/*`, `03-IaC/*` kalan 5, `07-Observability/*`, `08-Security/*`, …) → `16-Cheatsheets/` (kalan 8) → `17-Templates/` index'leri → en son `21-Field-Notes/` | 9+ |
