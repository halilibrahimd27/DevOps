...
| 71+ | sonra kalan 00-21 deep-dive'ları klasör sırasıyla (`10-Databases-Production` kalan 5: `HA-Patroni-Stolon`+`Monitoring-Postgres`+`Operator-Patterns`+`Postgres-Production-Guide`+`StatefulSet-vs-Operator`, sonra `11-SRE/*`, `12-FinOps` tam, `13-*`, …) → `16-Cheatsheets/` (kalan 8) → `17-Templates/` index'leri → en son `21-Field-Notes/` | 15+ |
