...
| **P4** | Kalan içerik | 🟡 **slice-6 hazır** (2026-07-24) — 5 `.en.md`, `05-Kubernetes/*` kalan 5 (#26–30): `05-Kubernetes/HPA-VPA-KEDA`, `05-Kubernetes/Multi-Tenancy-Patterns`, `05-Kubernetes/Production-Checklist`, `05-Kubernetes/Resource-Limits-Guide`, `05-Kubernetes/Upgrade-Strategy` → **`05-Kubernetes` artık tam twin (7/7)**. 5 paralel çeviri subagent, dosya başına bir (sonnet), oturmuş genişletilmiş ruleset + aynı-klasör gold-standard referans (`05-Kubernetes/Debugging-Pods.en.md`) → remediation gerekmedi. Plain/untagged blok prose çevrildi (HPA code-yorumu `# CPU 70% target`/`# fast scale up`/`# wait 5 min`, Production-Checklist `[ ]` label metni + anchor A2/D4 sabit, Upgrade timeline `Hafta`→`Week`/`Adım`→`Step`; `%40`→`40%`, `dk`→`min`, `hafta`→`week`); K8s QoS class adları (`Guaranteed`/`Burstable`/`BestEffort`) + status/reason string'leri (`OOMKilled`/`Evicted`/`Pending`/`CrashLoopBackOff`) + CRD/kind (`HorizontalPodAutoscaler`/`VerticalPodAutoscaler`/`ScaledObject`/`PodDisruptionBudget`/`ResourceQuota`/`LimitRange`/`NetworkPolicy`) + KEDA/vcluster/Capsule tenancy araçları + unit (`250m`/`512Mi`/`1Gi`) + `{{...}}` + `Europe/Istanbul` tz + komut + link-target verbatim. Bağımsız orchestrator paritesi: başlık 5/5 (36/34/74/42/60), tablo 5/5 (23/18/36/23/13), fence 5/5 (40/26/62/34/34), satır deltası -1/0/0/0/+1 (prose-wrap), gerçek Türkçe kalıntısı **0** (article body-TR=0, 5/5; `[ışğİŞĞ]` excl path/Istanbul + diakritiksiz TR-fonksiyon-kelime temiz), link locale-eksiz (0 sızıntı), positioning/pazarlama (TR+EN) hit = kaynak-sadık FP (`Guaranteed` K8s QoS class + Multi-Tenancy:246 `ROI yüksek müşteri için` kaynakta var; qa `22-Learning-Path/` scope dışı). qa exit 0, iki-locale build hatasız, 5 sayfa `/en/05-Kubernetes/…/` İngilizce render (EN-marker 123/77/51/61/99, body-TR=0), `_planning` sızmadı. EN kapsama %32.3 → %33.8 (113/334). **Slice-1..5 önceki turlar.** **Kalan P4 çok turlu** — sıra slice-7 (`04-Containers/*` kalan 6'nın ilk 5 → `03-IaC/*` → kalan 00-21). |
...
| 31+ | `04-Containers/*` kalan 6 (README twin'li; ilk 5: BuildKit-Tips, Container-vs-WASM, Distroless-and-Chainguard, Dockerfile-Best-Practices, Image-Signing-Cosign → slice-7; Multi-Stage-Builds → slice-8), `03-IaC/Terraform-Best-Practices.md` → sonra kalan 00-21 → `16-Cheatsheets/` → `17-Templates/` → `21-Field-Notes/` | 7+ |
