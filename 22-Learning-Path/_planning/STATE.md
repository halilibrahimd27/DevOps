...
Aşama B eşiği (%60) yaklaşıyor (%56.0) — P4 çok turludur, **§14.1.3 dosya-seviyesi dilim** (dilim başına ~5 dosya, klasör kapanışında daha az).

1. **P4 slice-22 = SONRAKİ DOSYALAR (deterministik, I18N-COVERAGE "P4 — dilim planı" #105+):
   `17-Templates/` alt-klasör index README'leri, klasör/alfabetik sırayla ilk 5** (`dockerfiles/README`,
   `github-actions/README`, `gitignore/README`, `kubernetes/README`, `kyverno-policies/README`;
   17-Templates kök `README`[P2] zaten twin; `find 17-Templates -name '*.md'` +
   `find 17-Templates -name '*.en.md'` ile hangi index zaten twin, hangileri kaldı teyit et — sonraki turda).
   `00-16` (16-Cheatsheets slice-21 ile 10/10) zaten tam twin → atlanır.
   **Sıra kaynağı:** I18N-COVERAGE.md → "P4 — dilim planı" tablosu #105+ (`17-Templates/` alt-klasör
   index README'leri → kalan `prometheus-rules/README`+`terraform/README`+`runbooks/{postmortem,runbook}-template`
