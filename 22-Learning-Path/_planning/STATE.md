...
Aşama B eşiği (%60) yakın (%58.7, 4 site sayfası kaldı) — P4 çok turludur, **§14.1.3 dosya-seviyesi dilim** (dilim başına ~5 dosya, klasör kapanışında daha az).

1. **P4 slice-24 = SONRAKİ DOSYALAR (deterministik, I18N-COVERAGE "P4 — dilim planı" #114+):
   en son P4 klasörü `21-Field-Notes/`** (00-17 tam twin; 18-20 rehber/patika P1/P2'de kapsandı → kalan
   yalnız `21-Field-Notes/`). `find 21-Field-Notes -name '*.md'` + `find 21-Field-Notes -name '*.en.md'` ile
   hangi `.md`'lerin twin'siz olduğunu teyit et (kurulum notları — Proxmox/Wazuh/K8s install/Ansible;
   klasör büyükse ~5 dosyalık dilimlere böl, her tur bir dilim). `00-17` zaten tam twin → atlanır.
   **Sıra kaynağı:** I18N-COVERAGE.md → "P4 — dilim planı" tablosu #114+ (`21-Field-Notes/` = P4'ün son klasörü).
   **Her tur bir dilim; nereye gelindiğini STATE + I18N-COVERAGE dilim tablosunda işaretle. Klasör kapanınca
   Aşama B (%60 eşiği) değerlendirmesi tetiklenir — ama Aşama B'yi bu turda YAPMA, koşul STATE'te.**
