...
`21-Field-Notes/` **kısmi (5/14)** [slice-24: README+ansible/2+kubectl/2; kalan network/1+system/6+terraform/2=9 → slice-25/26].
🎯 **Aşama B eşiği (%60) AŞILDI (%60.2, 201/334)** — ama Aşama B (EN varsayılan flip) **bu turda YAPILMADI** (§14.1.2 bir-slice/tur + §Faz-1(c) "şimdi yapma"; `21-Field-Notes` tam twin olunca kendi turunda değerlendir/yürüt — koşul/gerekçe `Açık kararlar`da). P4 çok turludur, **§14.1.3 dosya-seviyesi dilim** (dilim başına ~5 dosya, klasör kapanışında daha az).

1. **P4 slice-25 = SONRAKİ DOSYALAR (deterministik, I18N-COVERAGE "P4 — dilim planı" #119+):
   `21-Field-Notes/` KALAN 9** (slice-24 = README+ansible/2+kubectl/2 twin'lendi → 5/14; kalan:
   `network/network-segmentation-wazuh-siem` [1256 satır — tek başına dilim olabilir] + `system/` 6
   [`external-access-solutions`/`github-actions-pipeline-setup`/`inventory-management-example`/
   `kubernetes-cluster-installation`/`production-ready-repo-layout` + `devops-certification-roadmap`
   =22-satır yönlendirme stub'ı] + `terraform/` 2 [`modules-create-vm`/`proxmox-configuration`;
   `8.8.8.8`/`8.8.4.4` = Google well-known resolver, doküman-örneği, yeni IP DEĞİL]).
   `find 21-Field-Notes -name '*.en.md'` ile 5 twin teyit et, kalan 9 için klasör sırasıyla ~5'lik
   dilim seç (network büyük → tek başına ya da +system birkaç). **Saha-notu ruleset (slice-24 emsali):**
   yapı byte-korunur, komut/YAML/HCL/jsonpath/echo-jsonpath verbatim, yalnız `description`+H1+`Saha notu`
   callout+`#` yorum+kullanıcı-yüzü echo/prose çevrilir; küçük dosyalar orchestrator-direct (`cp`+Edit),
   büyükler (network 1256/production-ready 1497) subagent. `00-17` zaten tam twin → atlanır.
   **Sıra kaynağı:** I18N-COVERAGE.md → "P4 — dilim planı" tablosu #119+ (`21-Field-Notes/` = P4'ün son klasörü).
   **Her tur bir dilim; nereye gelindiğini STATE + I18N-COVERAGE dilim tablosunda işaretle.
   `21-Field-Notes` kapanınca (slice-25/26 sonu) Aşama B (%60 zaten AŞILDI) değerlendirmesi KENDİ TURUNDA —
   ama Aşama B flip'ini o tur gelene kadar YAPMA, koşul/gerekçe `Açık kararlar`da.**
...
> 🎯 **BEKLEYEN MİLESTONE (bu tur, slice-24) — Aşama B eşiği %60 AŞILDI (%60.2, 201/334); flip henüz YAPILMADI.**
> BUILD-PROMPT §Faz-1(c): "EN kapsama ≥ %60 olunca EN varsayılan yapılır (`default: true` en'e taşınır,
> TR `/tr/`'ye iner) — **ama bunu şimdi yapma.**" Otonom sözleşme §14.1.2 de bir-slice/tur diyor. Slice-24 ile
> eşik geçildi ama flip **bilerek ertelendi**, çünkü: (a) flip ayrı ve büyük bir operasyon (mkdocs i18n
> `default` locale taşı + kök nav + `nav_translations` + ~924 iç link locale-eksiz doğrulaması + iki-locale
> re-test + rozet/landing URL'leri); (b) `21-Field-Notes` henüz 5/14 → önce P4'ü tam bitir (slice-25/26),
> **sonra** Aşama B'yi kendi turunda yürüt (kısmi-kapsama sırasında default flip'i temiz değil).
> **Karar:** P4 (21-Field-Notes) tamamlandığında Aşama B'yi ayrı bir tur/faz olarak ele al; o zamana kadar
> Aşama A (TR default) korunur. Bu bir tıkanma DEĞİL — planlı erteleme, `.local/PAUSE` gerekmez.
>
