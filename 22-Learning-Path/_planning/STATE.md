...
`14-Sustainability/` artık **tam twin'li (6/6)** (bu tur Efficiency-Practices + Green-Software-Principles +
Measuring-Software-Carbon + Region-Selection ile kapandı — README[P2] + Carbon-Aware-Computing[s17] önceki turlar);
`15-AI-LLMOps/` **2/8** (README[P2] + bu tur AI-Augmented-Operations; kalan 6 → slice-19+).
Aşama B eşiği (%60) hâlâ uzak (%51.8) — P4 çok turludur, **§14.1.3 dosya-seviyesi dilim** (dilim başına 5 dosya).

1. **P4 slice-19 = SONRAKİ 5 DOSYA (deterministik, I18N-COVERAGE "P4 — dilim planı" #91+): kalan 00-21
   klasör sırasıyla → `15-AI-LLMOps/*`'in henüz `.en.md`'siz ilk 5 deep-dive'ı** (`ls 15-AI-LLMOps/*.md`
   ve `ls 15-AI-LLMOps/*.en.md` ile teyit: README[P2] + AI-Augmented-Operations[s18] zaten twin →
   kalan LLM-in-Production/Model-Cost-Optimization/Prompt-Engineering-for-Ops/RAG-Architecture/
   Safety-and-Guardrails/Self-Hosted-LLM = 6; **ilk 5 bu dilim**, 6.'sı [Self-Hosted-LLM] → slice-20'ye
   VEYA dilimi `16-Cheatsheets/*` ilk 1 ile 5'e tamamla — klasör-sırası kararı sonraki turda `ls` ile).
   `11-SRE` + `12-FinOps` + `13-Platform-Engineering` + `14-Sustainability` zaten tam twin → atlanır.
   **Sıra kaynağı:** I18N-COVERAGE.md → "P4 — dilim planı" tablosu #91+ (`15-AI-LLMOps/*` kalanı →
   sonra kalan 00-21 klasör sırasıyla →
