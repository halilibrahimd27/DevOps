...
1. **SIRADAKI = Aşama B — EN varsayılan flip (P4 twin işi BİTTİ; ön koşullar tam: %62.9 ≥ %60 + tüm 00-21 tam twin).**
   Bu bir çeviri turu DEĞİL — tek seferlik bir i18n konfigürasyon operasyonu; kendi turunda yürüt:
   (a) `mkdocs.yml` `i18n` plugin: `default` locale'ini `tr`→`en`'e taşı; TR `/tr/` altına iner
   (`docs_structure: suffix` + `fallback_to_default: true` korunur; `awesome-pages` yine `i18n`'den ÖNCE);
   (b) `build-docs.sh` `.pages` başlık/`nav_translations` iki-locale doğru mu (kök nav sırası korunur);
   (c) **~924 iç link locale-eksiz teyidi** (`grep -rn '\.\(en\|tr\)\.md' 00-*..22-*` → link hedeflerinde
   locale eki 0 olmalı ki fallback doğru çözülsün); (d) rozet/`site_url`/`repo_url`/`docs/about` EN landing'e göre;
   (e) iki-locale `bash scripts/build-docs.sh` + `python3 -m mkdocs build --clean` re-test (TR artık `/tr/`
   altında render, EN root default, `_planning` sızmaz, kırık link 0); (f) `python3 .local/qa.py` exit 0.
   **Görünür ama geri-döndürülebilir bir değişiklik** → tek turda tamamla; öncesinde mevcut `default: tr`
   durumunu STATE'e yaz. Tam adım listesi + "neden ayrı tur" gerekçesi `Açık kararlar` + I18N-COVERAGE "Aşama".
...
> 🎯 **AKTİF MİLESTONE (slice-26, P4 BİTTİ) — Aşama B ön koşulları TAM: eşik %60 AŞILDI (%62.9, 210/334) + tüm 00-21 tam twin; flip SIRADAKI TUR, henüz YAPILMADI.**
...
> re-test + rozet/landing URL'leri); (b) `21-Field-Notes` **artık 14/14 → P4 tam bitti** (slice-24/25/26),
> yani "önce tam twin, sonra flip" koşulu **karşılandı** (kısmi-kapsama sırasında default flip'i temiz olmazdı).
> **Karar (güncel):** P4 (21-Field-Notes dahil tüm 00-21) TAMAMLANDI → **Aşama B artık SIRADAKI tur/faz.**
> Yine de bu turda flip YAPILMADI (§14.1.2 bir-slice/tur: bu tur slice-26 twin işi tamamlandı). Sıradaki tur
> Aşama B flip'ini `Sıradaki adım` madde 1'deki (a)–(f) adımlarıyla tek turda yürütür; o ana kadar Aşama A
> (TR default) korunur. Bu bir tıkanma DEĞİL — planlı sıralı geçiş, `.local/PAUSE` gerekmez.
>
