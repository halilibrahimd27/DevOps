...
## Blok C (C0–C4) — AÇIK (sonraki tur)

| ID | Kat | Yer | Bulgu | Öneri | Durum |
|---|---|---|---|---|---|
| C-01 | b/f | C0:31-36,78-91 | C0 Bash-only öğrenciye Python sözdizimini (`with`, dict, f-string, argparse) öğretmeden "dışarı çıkmadan lab'ı yapabilirsin" diyor; tek gerçek kaynak (resmi tutorial) "isteğe bağlı". | Gövdeye minimal Python sözdizimi köprüsü ekle VEYA resmi tutorial'ı zorunlu "Önce oku" (4 alan) yap ve iddiayı yumuşat. | ⬜ |
| C-02 | a | C2:41,53 | `semver` tanımsız; Glossary'de yok. | İlk kullanımda gloss + Glossary. | ⬜ |
| C-03 | g | C0:33-35 | `docs.python.org/3/tutorial` referansı 4 alanı inline doldurmuyor, sorumluluğu okuyucuya devrediyor; C-01 nedeniyle fiilen zorunlu kaynağa "isteğe bağlı" diyor. | 4-sütun tablo satırına çevir, zorunlu/opsiyonel statüsünü netleştir. | ⬜ |
| C-04 | a | C3:32 | `LocalStack` ilk geçişte tanımsız; Glossary'de yok. | Gloss ("AWS'i yerelde taklit eden öykünücü") + Glossary. | ⬜ |
| C-05 | b | C0:52,128 | Kendini-test Q3 "pipeline dostu" — CI/pipeline C2'de (C0'dan sonra) öğretiliyor. | Soruyu henüz-öğretilmemiş terime dayanmayacak şekilde yaz veya "(C2'de göreceksin)" notu. | ⬜ |
| C-06 | c | C4:52 | Son kabul kriteri "VPC/IAM/compute kavramlarını kendi cümlelerinle tanımlayabiliyorsun" — yazılı kanıt şartı yok (öznel); L12 lab bunu `report.txt`'e yazdırıyor, modül kriteri lab'dan gevşek. | "...yazdın (L12 `report.txt`)" biçimine çevir. | ⬜ |
| C-07 | a | C4:51,72 | `NAT`/`egress`/`free tier` tanımsız (Glossary'de NAT/egress yok). | Tek kelimelik gloss + Glossary. | ⬜ |

## Blok D (D1–D5) — AÇIK (sonraki tur) — güvenlik ipliği kritik

| ID | Kat | Yer | Bulgu | Öneri | Durum |
|---|---|---|---|---|---|
| D-01 | h/d | D1:54 + L13 networkpolicy.yaml + L13 verify.sh:33 | D1'in "ilk günden NetworkPolicy — yetkisiz erişimin kesildiğini göster" kabul kriteri fiilen doğrulanmıyor: L13 starter "kindnet NetworkPolicy uygular" diye koşulsuz iddia ediyor (sürüm-bağımlı; klasik kindnet zorlamaz, Calico gerekir), verify.sh yalnız report.txt'te "engellendi" kelimesi grep'liyor. Engelleme no-op olsa da lab geçer. | kind'e NetworkPolicy uygulayan CNI kuran adım VEYA min kind sürümü pin; verify.sh'a probe Pod'unun gerçekten reddedildiğini (canlı `wget` exit) sınayan guard'lı kontrol. | ⬜ |
| D-02 | h/f | D4:16-17 vs :39; L16 | D4 🎯 "cluster yalnızca imzalı image kabul eder" der ama kabul kriteri yalnız "niçin reddetmeli — yazılı gerekçe"ye iniyor; L16 yalnız `cosign sign/verify` yapıp cluster tarafını atlıyor. Admission controller/Kyverno A–D'de hiç öğretilmiyor. | 🎯'i kabul kriteriyle uyumla (ya Kyverno/Sigstore admission lab'a ekle, ya 🎯'i "reddetme gerekçesi + admission nerede devreye girer"e çek) + 1-2 cümle admission köprüsü + "Önce oku"ya Policy-as-Code-OPA-Kyverno.md. | ⬜ |
| D-03 | h/a | D1:27-35 (🌉 Köprü) | Köprü Pod/Deployment/Service/Ingress'e kavram veriyor ama modülün kimliği olan RBAC ve NetworkPolicy'ye köprü yok — bu iki kavramın modül-içi tek varlığı bir okuma linki + kabul kriteri. | Köprüye RBAC ("kim neyi yapabilir — en az yetki") ve NetworkPolicy ("default-deny") için Pod/Service ile eşit 1-2 cümlelik tanım. | ⬜ |
| D-04 | a | D1:23,44,80; D5:32 (Glossary) | `kind` (ve `k3s`) açılımsız/tanımsız; Glossary'de yok; İngilizce "kind" ile karışıyor. | İlk kullanımda "kind = Kubernetes-in-Docker, yerel tek-makine cluster" + Glossary `kind`/`k3s`. | ⬜ |
| D-05 | d/f | D2:42; L14 | D2 kabul kriteri "drain'de en az bir replica ayakta — kanıt" ister; L14 yalnız `kubectl get pdb` yaptırıyor (gerçek drain yok); tek-node kind'de gerçek drain sorunlu. | Kriteri "PDB uygulandı + `get pdb` + drain davranışını yazılı açıkla"ya hizala VEYA L14'e çok-node kind + gerçek drain. | ⬜ |
| D-06 | f | D1:6,13,21 | D1 önkoşulu frontmatter'da `[C1,C2,C3]` ve anlatı C3/C4'e yaslanıyor; MODULE-SPEC+DAG `[C1,C2]` diyor ve D1 "bulut/para gerekmez". C3 gerekçesiz sert önkoşul, spec ile çelişki. | Önkoşulu `[C1,C2]`'ye indir (spec/DAG uyumu); anlatıyı C3/C4 zorunluluğu ima etmeyecek şekilde yumuşat. | ⬜ |
| D-07 | b | D1:33,65,74 | Ingress'in çalışması için ingress controller gerektiği söylenmiyor; `taint/toleration` tanımsız. | Ingress'e "kural tek başına yetmez, controller gerekir" notu; taint/toleration'a tek cümle/Glossary. | ⬜ |
| D-08 | g | D1–D5 "Önce oku" | Dış link yok (ihlal yok); ama "Önce oku" tabloları 3 alanlı, §9'un "dönünce doğrulama" disiplini iç okumalara taşınmamış. | Karar (A-05 ile aynı): §9 4-alan = dış yönlendirilmiş okuma; iç deep-dive "Önce oku" kapsam dışı. STUDY-METHOD §9'da istisna zaten belgeli mi kontrol et, değilse netleştir. | ⬜ |
