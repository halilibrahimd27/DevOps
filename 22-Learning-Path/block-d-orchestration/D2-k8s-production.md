...
## 💥 Kırık lab
👉 [`labs/broken/K05-oomkilled-probe/`](../labs/broken/K05-oomkilled-probe/README.md) — Belirti: "Pod sürekli yeniden
...
- [ ] Bir **PodDisruptionBudget** uygulandı (`kubectl get pdb` `ALLOWED DISRUPTIONS` gösteriyor); gönüllü kesinti (drain) sırasında PDB'nin en az bir replica'yı niçin ayakta tuttuğunu **yazılı açıklıyorsun** (tek-node kind'de gerçek drain yapılamaz — davranışı `min-available`/`maxUnavailable` üzerinden anlatırsın)
