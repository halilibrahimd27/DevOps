---
description: "Guide to a network segmentation architecture split into DMZ, application, and management zones with Wazuh SIEM integration; VLAN/subnet plan and security diagram."
tags:
  - Field Notes
  - Security
  - Networking
  ...
  - Compliance
---
# 🔒 Network Segmentation and Wazuh SIEM Integration Guide

> ℹ️ **Placeholder note:** All IP/subnet values in this guide (`192.168.x.x` etc.)
> **are RFC 1918 example addresses** — not real infrastructure. They are kept
> concrete to illustrate the segmentation scheme; adapt them to your own VLAN/subnet plan.
...
## 🌐 **1. PROXMOX NETWORK SEGMENTATION SETUP**

### Creating Proxmox Network Bridges

#### 1.1 Network Configuration via Proxmox Web UI:
```bash
# Create bridges on the Proxmox host
# DMZ Bridge
auto vmbr1
...
```bash
# Reconfigure your existing K8s cluster:
...
# Production Cluster A (192.168.21.0/24) - to be newly created
- k8s-prod-a-master: 192.168.21.10
- k8s-prod-a-worker1: 192.168.21.11
- k8s-prod-a-worker2: 192.168.21.12

# Production Cluster B (192.168.22.0/24) - to be newly created  
...
## 🛡️ **2. WAZUH SIEM SETUP**

### 2.1 Wazuh Cluster Setup

#### Wazuh Manager VM (192.168.30.10):
```bash
# Ubuntu 22.04 installation and update
...
# Pre-configured indexer installation
...
### 2.2 Creating Wazuh SSL Certificates

```bash
# Run the certificate creation tool on Wazuh Manager
sudo /usr/share/wazuh-indexer/plugins/opensearch-security/tools/install_demo_configuration.sh -y

# Copy the certificates to the other nodes
# (Repeat this step for each VM)
```

---

## 🔧 **3. WAZUH AGENT SETUP ON KUBERNETES CLUSTERS**

### 3.1 Wazuh Agent Setup on Each K8s Node

#### On All Kubernetes Nodes (Test + Prod-A + Prod-B):
```bash
...
# Assign a different group for each node:
...
### 3.2 Wazuh Integration for Kubernetes Events

#### K8s API Server Monitoring:
```yaml
# k8s-audit-policy.yaml (on each master node)
...
#### API Server Audit Log Configuration:
```bash
# Edit /etc/kubernetes/manifests/kube-apiserver.yaml on each master node
...
```xml
<!-- On each K8s node: /var/ossec/etc/ossec.conf -->
<ossec_config>
...
## 🔥 **4. WAZUH RULES AND DETECTION CONFIGURATIONS**
...
## 📊 **5. WAZUH DASHBOARDS AND MONITORING**

### 5.1 Custom Kubernetes Dashboard

#### Kubernetes Overview in Wazuh Dashboard:
```json
{
  "version": "4.5.4",
  "objects": [
...
## 🔧 **6. NETWORK FIREWALL RULES AND ACCESS CONTROL**

### 6.1 UFW Firewall Rules (For Each VM)
...
## 🚨 **7. INCIDENT RESPONSE AND AUTOMATED REMEDIATION**
...
## 🔒 **9. COMPLIANCE AND SECURITY BASELINES**
...
### 9.2 PCI DSS Compliance (If a Payment System Exists)
...
## 📈 **10. MONITORING DASHBOARDS AND REPORTING**
