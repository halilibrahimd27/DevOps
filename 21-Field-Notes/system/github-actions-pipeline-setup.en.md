---
description: "GitHub Actions CI/CD pipeline setup guide: repository structure, workflow files, Kustomize-based k8s base/staging/production environments and the Docker flow."
tags:
  - Field Notes
  - CI/CD
  - Kubernetes
...
## 📁 **STEP 1: Organize the Repository Structure**
...
## 🐳 **STEP 2: Prepare the Sample Application and Dockerfile**

### Simple Node.js Application (`src/app.js`):
```javascript
...
## ⚙️ **STEP 3: Prepare the Kubernetes Manifests**
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: app
        image: ghcr.io/USERNAME/devops-app:latest # Replace this with your own username
        ports:
        - containerPort: 3000
        env:
        - name: APP_VERSION
...
spec:
  rules:
  - host: devops-app.local # local host for testing
    http:
      paths:
      - path: /
        pathType: Prefix
...
#### 1. Kubeconfig Files:
```bash
# Base64-encode the kubeconfig for the staging cluster
cat ~/.kube/config | base64 -w 0
# Add the output as KUBECONFIG_STAGING

# For production cluster-a (repeat the same process with your production cluster)
# Add as KUBECONFIG_PROD_A

# For production cluster-b (once the second production cluster is set up)
# Add as KUBECONFIG_PROD_B
```

#### 2. Environment Variables (Optional):
- `SLACK_WEBHOOK_URL`: For notifications
- `DOCKER_REGISTRY_PASSWORD`: For extra security
...
```bash
# Create a new repo on GitHub: my-devops-project
...
### 2. Install the Ingress Controller on Kubernetes:
```bash
...
### 3. Test Deployment:
```bash
# Push to the develop branch to trigger a staging deploy
...
### 4. Local Test:
```bash
# Test once staging has been deployed
kubectl get pods -n staging
...
```bash
# If you see an error in the Actions tab, check the logs
# Common errors:
# 1. Kubeconfig secret missing/incorrect
# 2. Image tag formatting errors
# 3. Kubernetes namespace missing
